# optmizing the code using precision, kernel flash attention and pytorch tricks
...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        """
        normally, weight decay is implemented for 2D tensors:  matmuls and embeddings (not position embd tho)
        and not on layernorms or biases. 
        This function separates decay and nondecay params for transparency + fuses kernels to reduce gpu
        overheads if device = "cuda"
        """
...
if __name__ == "__main__":
    ...
    for i in range(50):
        ...
        print(f"step {i}: loss = {loss.item()}, dt: {dt:.2f} ms, {tokens_per_sec:.2f} tokens/sec")
